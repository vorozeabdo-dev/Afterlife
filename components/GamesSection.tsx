...
const GamesSection: React.FC = () => {
  ...
  return (
    <section id="игры" className="py-20 bg-cyber-black relative overflow-hidden">
       {/* Background Grid */}
       ...
       <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="mb-16">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  {pcGames.map((game) => (
                      <div key={game.id} className="group relative aspect-[3/4] overflow-hidden rounded-sm border border-white/10 hover:border-cyber-pink transition-colors cursor-pointer">
                          <img 
                              src={game.image} 
                              alt={game.title} 
                              className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 group-hover:grayscale-0 grayscale-[0.5]"
                          />
          ...
          <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  {consoleGames.map((game) => (
                      <div key={game.id} className="group relative aspect-[3/4] overflow-hidden rounded-sm border border-white/10 hover:border-cyber-cyan transition-colors cursor-pointer">
                          <img 
                              src={game.image} 
                              alt={game.title} 
                              className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 group-hover:grayscale-0 grayscale-[0.5]"
